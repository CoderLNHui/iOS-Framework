...
@class NSURLSession; //请求
@class NSURLSessionDataTask; //数据请求、离线下载                /* DataTask objects receive the payload through zero or more delegate messages */
@class NSURLSessionUploadTask; //上传任务               /* UploadTask objects receive periodic progress updates but do not return a body */
@class NSURLSessionDownloadTask;//下载任务            /* DownloadTask objects represent an active download to disk.  They can provide resume data when canceled. */
@class NSURLSessionStreamTask;//流任务 ios9之后出现的,用于TCP/IP流              /* StreamTask objects may be used to create NSInput and NSOutputStreams, or used directly in reading and writing. */
...
@property (readonly, retain) NSOperationQueue *delegateQueue;// 队列
...
/**
 允许后台任务在性能最优状态下进行(当电量不足或者是蜂窝时将不进行,后台任务建议设置该属性)
 */
