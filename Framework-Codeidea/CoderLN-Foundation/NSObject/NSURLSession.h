...
#pragma mark - 相关类
@class NSURLSession; // 会话请求基类
@class NSURLSessionDataTask; // 数据请求任务（GET、POST请求、断点下载）                /* DataTask objects receive the payload through zero or more delegate messages */
@class NSURLSessionUploadTask; // 上传请求任务               /* UploadTask objects receive periodic progress updates but do not return a body */
@class NSURLSessionDownloadTask;// 下载请求任务            /* DownloadTask objects represent an active download to disk.  They can provide resume data when canceled. */
...
@class NSURLSessionConfiguration;// Session配置信息
@protocol NSURLSessionDelegate;// Session代理

@class NSURLSessionTaskMetrics;// 请求统计(对发送请求/DNS查询/TLS握手/请求响应等各种环节时间上的统计)
@class NSDateInterval;// 日期间隔
 ...
 */
#pragma mark -创建一个指定配置信息的会话对象
+ (NSURLSession *)sessionWithConfiguration:(NSURLSessionConfiguration *)configuration;

#pragma mark -创建一个指定配置信息、代理和代理方法执行队列的会话对象
+ (NSURLSession *)sessionWithConfiguration:(NSURLSessionConfiguration *)configuration delegate:(nullable id <NSURLSessionDelegate>)delegate delegateQueue:(nullable NSOperationQueue *)queue;

#pragma mark - Session属性
#pragma mark -代理方法执行的队列
 ...
 */
#pragma mark -app定义的对于该session的描述
...
#pragma mark -任务全部完成后释放掉session，后置nil
/**
 ...
 另外session失效后会调用URLSession:​did​Become​Invalid​With​Error:方法, 之后session释放对代理的强引用.
 */
 ...
 */
#pragma mark -取消所有未完成的任务并销毁session
- (void)invalidateAndCancel;

#pragma mark -清空cookies缓存和证书存储，移除所有磁盘文件，清理正在执行的下载任务，确保未来能响应一个新的socket请求
...
#pragma mark -清除硬盘上的cookies缓存和证书，清理暂时的缓存，确保未来能响应一个新的TCP请求
...
#pragma mark -异步调用session中所有upload, download, data tasks的completion回调.
...
#pragma mark -异步调用SessionTask的completion回调.
...
#pragma mark - NSURLSessionTask 会话任务 👣
/*
 * NSURLSessionTask objects are always created in a suspended state and
 * must be sent the -resume message before they will execute.
 */

#pragma mark -添加dataTask数据任务
/* Creates a data task with the given request.  The request may have a body stream. */
// 获取指定URLRequest内容
- (NSURLSessionDataTask *)dataTaskWithRequest:(NSURLRequest *)request;

// 获取指定URLRequest内容, 在completionHandler中Block回调处理数据. 该方法会绕过代理方法(除了身份认证调用的代理方法)
...
// 获取指定URL内容, 在completionHandler中处理数据. 该方法会绕过代理方法(除了身份认证调用的代理方法)
- (NSURLSessionDataTask *)dataTaskWithURL:(NSURL *)url completionHandler:(void (^)(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error))completionHandler;


#pragma mark - 添加uploadTask上传任务
...
// 通过HTTP请求发送指定文件给指定URL, 在completionHandler中处理数据. 该方法会绕过代理方法(除了身份认证调用的代理方法)
...
// upload convenience method.
// 通过HTTP请求发送data给指定URL, 在completionHandler中处理数据. 该方法会绕过代理方法(除了身份认证调用的代理方法)
- (NSURLSessionUploadTask *)uploadTaskWithRequest:(NSURLRequest *)request fromData:(nullable NSData *)bodyData completionHandler:(void (^)(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error))completionHandler;



#pragma mark - 添加downloadTask下载任务
/* Creates a download task with the given request. */
// 下载指定URLRequest内容
- (NSURLSessionDownloadTask *)downloadTaskWithRequest:(NSURLRequest *)request;

// 下载指定URLRequest内容, 在completionHandler中处理数据. 该方法会绕过代理方法(除了身份认证调用的代理方法)
...
// 下载指定URL内容, 在completionHandler中处理数据. 该方法会绕过代理方法(除了身份认证调用的代理方法)
...
// 创建一个之前被取消/下载失败的download task, 在completionHandler中处理数据. 该方法会绕过代理方法(除了身份认证调用的代理方法)
 ...
 */
// 通过给定的networkservice建立双向TCP/IP连接
...
#pragma mark - NSURLSessionTask 会话任务；是一个抽象类，本身不能使用只能使用它的子类 👣
// 会话任务状态
typedef NS_ENUM(NSInteger, NSURLSessionTaskState) {
    NSURLSessionTaskStateRunning = 0,//运行                     /* The task is currently being serviced by the session */
    NSURLSessionTaskStateSuspended = 1,//暂停
    NSURLSessionTaskStateCanceling = 2,//取消                 /* The task has been told to cancel.  The session will receive a URLSession:task:didCompleteWithError: message. */
    NSURLSessionTaskStateCompleted = 3,//完成                  /* The task has completed and the session will receive no more delegate notifications */
...
#pragma mark -DownloadTask取消是可以恢复，将resumeData保存用于恢复
...
#pragma mark - NSURLSessionStreamTask 流任务 👣
...
#pragma mark - NSURLSessionConfiguration 会话配置信息 👣
/*!
...
#pragma mark -默认配置 (使用磁盘缓存,用将证书存在用户的钥匙串)
@property (class, readonly, strong) NSURLSessionConfiguration *defaultSessionConfiguration;
#pragma mark -临时配置 (内存缓存不做磁盘缓存,也存储证书,它的信息存于RAM中)
@property (class, readonly, strong) NSURLSessionConfiguration *ephemeralSessionConfiguration;

#pragma mark -后台配置 (需要指定一个identifier用来后台重连session对象,做后台上传/下载,但是有一个独立进程来操作上传/下载)
...
#pragma mark -允许后台任务在性能最优状态下进行(当电量不足或者是蜂窝时将不进行,后台任务推荐设置该属性)
 ...
 */
#pragma mark -是另一个新的属性，该属性指定该 session 是否应该从后台启动。
@property BOOL sessionSendsLaunchEvents API_AVAILABLE(ios(7.0), watchos(2.0), tvos(9.0)) API_UNAVAILABLE(macos);

/* The proxy dictionary, as described by <CFNetwork/CFHTTPStream.h> */
#pragma mark -指定了 session 连接中的代理服务器。
//同样地，大多数面向消费者的应用程序都不需要代理，所以基本上不需要配置这个属性。
...
#pragma mark -指定了一组默认的可以设置请求（outbound request）的数据头
/**
...
#pragma mark -每端口的最大并发持久HTTP请求数目
/**
 注解：是 Foundation 框架中 URL 加载系统的一个新的配置选项。它曾经被 NSURLConnection 用于管理私有的连接池。现在有了 NSURLSession，开发者可以在需要时限制连接到特定主机的数量。
 */
@property NSInteger HTTPMaximumConnectionsPerHost;//

/* The cookie storage object to use, or nil to indicate that no cookies should be handled */
/**
 默认情况下会使用 NSHTTPCookieShorage 的 +sharedHTTPCookieStorage 这个单例对象，这与 NSURLConnection 是相同的。
 */
#pragma mark -存储了session所使用的cookie
@property (nullable, retain) NSHTTPCookieStorage *HTTPCookieStorage;

/* The credential storage object, or nil to indicate that no credential storage is to be used */
/**
  默认情况下会使用 NSURLCredentialStorage 的 +sharedCredentialStorage 这个单例对象，这与 NSURLConnection 是相同的。
 */
#pragma mark -存储了 session 所使用的证书
@property (nullable, retain) NSURLCredentialStorage *URLCredentialStorage;

/* The URL resource cache, or nil to indicate that no caching is to be performed */
/**
 默认情况下会使用 NSURLCache 的 +sharedURLCache 这个单例对象，这与 NSURLConnection 是相同的。
 */
#pragma mark -session缓存策略
...
#pragma mark - NSURLSessionDelegate 处理鉴权、后台下载任务完成通知 👣
/*
 * Disposition options for various delegate messages
 */
#pragma mark -NS_ENUM 请求方式
...
#pragma mark -NS_ENUM 服务器传给我们的证书类型
...
#pragma mark -NS_ENUM 响应处理方式
typedef NS_ENUM(NSInteger, NSURLSessionResponseDisposition) {
    NSURLSessionResponseCancel = 0,//取消响应，默认                                      /* Cancel the load, this is the same as -[task cancel] */
    NSURLSessionResponseAllow = 1,//接收响应                                      /* Allow the load to continue */
    NSURLSessionResponseBecomeDownload = 2,//转换为下载任务                              /* Turn this request into a download */
    NSURLSessionResponseBecomeStream//转换为流任务 API_AVAILABLE(macos(10.11), ios(9.0), watchos(2.0), tvos(9.0)) = 3,  /* Turn this task into a stream task */
} NS_ENUM_AVAILABLE(NSURLSESSION_AVAILABLE, 7_0);
 ...
 */
#pragma mark -当一个session遇到系统错误或者未检测到的错误的时候，就会调用这个方法。
 ...
 */
#pragma mark -应用在后台，而且后台所有下载任务完成方法会被调用。
// 我们在这里可以对session发起的请求做各种操作比如请求完成的回调等。
...
#pragma mark - NSURLSessionTaskDelegate 主要处理收到鉴权响应、任务结束(无论是正常还是异常) 👣
/*
 ...
 */
/**
 方法：URLSession:​task:​will​Perform​HTTPRedirection:​new​Request:​completion​Handler: 重定位响应
 ...
 */
#pragma mark -重定位响应，这也是所有task都有可能经历的一个过程, 如果response是HTTP重定位则session会调用该方法
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task
 ...
 */
#pragma mark -StreamTask：如果任务的数据是由一个stream发出的,session就会调用
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task
 ...
 */
#pragma mark -uploadTask：上传数据去服务器期间, 代理会周期性收到该方法回调并获得上传进度
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task
 ...
 */
#pragma mark -当task的统计信息收集好了以后，调用这个方法。
 ...
 */
#pragma mark -所有Sessiontask任务完成(成功 或 失败)后的回调
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task
...
#pragma mark - NSURLSessionDataDelegate 处理数据的接收、dataTask转downloadTask、缓存 👣
#pragma mark -调用流程：DataTask Delegate
/**
    1.对于一个data task来说, session会调用代理的 URLSession:​data​Task:​did​Receive​Response:​completion​Handler:​ 方法, 决定是否将一个data dask转换成download task, 然后调用completion回调继续接收data或下载data.
    2.如果你的app选择转换成download task, session会调用代理的 URLSession:​data​Task:​did​Become​Download​Task:​ 方法并把新的download task对象以方法参数的形式传给你. 之后代理不会再收到data task的回调而是转为收到download task的回调
    3.在服务器传输数据给客户端期间, 代理会周期性地收到 URLSession:​data​Task:​did​Receive​Data:​ 回调

    4.session会调用 URLSession:​data​Task:​will​Cache​Response:​completion​Handler:​ 询问你的app是否允许缓存. 如果代理不实现这个方法的话, 默认使用session绑定的Configuration的缓存策略.
 */
...
#pragma mark -1.当一个dataTask接收到服务器的响应调用。当所有信息都接收完毕以后，completionHandler会被调用。
/**
  它默认会取消网络请求或者把一个datatask转换为downloadtask。如果没有实现这个代理方法，我们也可以通过task的response属性获取到对应的数据。background模式的uploadtask不会调用这个方法。
    session 会话对象
    dataTask 请求任务
    response 响应头信息
 ...
 */
#pragma mark -2.当一个datatask转换为一个downloadtask以后会调用。
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask
 ...
 */
#pragma mark -3.接收服务器返回数据 (会被调用多次)
/**
    session 会话对象
    dataTask 请求任务
    data 本次下载的数据
 ...
 */
#pragma mark -4.询问你的app是否允许缓存.如果代理不实现这个方法的话,默认使用session绑定的Configuration的缓存策略
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask
...
#pragma mark - NSURLSessionDownloadDelegate 主要处理数据下载、数据进度通知 👣
#pragma mark -调用流程：downloadTask Delegate
/**
    1.对于一个通过download​Task​With​Resume​Data:​创建的下载任务, session会调用代理的 URLSession:​download​Task:​did​Resume​At​Offset:​expected​Total​Bytes:​方法.
    2.在服务器传输数据给客户端期间, 调用URLSession:​download​Task:​did​Write​Data:​total​Bytes​Written:​total​Bytes​Expected​To​Write:给用户传数据
        ...
        2.如果用户想要恢复下载, 把刚刚的resumeData以参数的形式传给 download​Task​With​Resume​Data:​ 方法创建新的task继续下载.
    3.如果download task成功完成了, 调用 URLSession:​download​Task:​did​Finish​Downloading​To​URL: 把临时文件的URL路径给你. 此时你需要剪切文件把它移动(NSFileManager moveItemAtURL:)到我们指定的位置路径fullPath.
 */
/*
 ...
 * still be called.
 */
#pragma mark -3.当下载完成的时候调用，需要把临时文件的URL路径剪切文件把它移动到我们指定的位置路径
/**
 location文件的临时存储路径tmp, 需要剪切文件把它移动(moveItemAtURL:)到我们指定的位置路径
 */
...
#pragma mark - ↑
#pragma mark - NSURLSessionStreamDelegate 流任务代理方法
 ...
 */
API_AVAILABLE(macosx(10.12), ios(10.0), watchos(3.0), tvos(10.0))
//作用：一个task从发出请求到收到数据过程中派生出的每个子请求
...
API_AVAILABLE(macosx(10.12), ios(10.0), watchos(3.0), tvos(10.0))
//作用：对发送请求的时间统计，内部装着许多transactionMetrics数组
