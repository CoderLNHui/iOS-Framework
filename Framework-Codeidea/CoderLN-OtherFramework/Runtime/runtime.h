 ...
 *  with an instance of the class, or \c NULL if \e cls is \c Nil.
 */
#pragma mark - 获取类中的方法的实现,该方法的返回值类型为struct
 ...
 */
#pragma mark - 为类添加新的方法，如果方法该方法已存在则返回NO
 ...
 */
#pragma mark - 替换类中已有方法的实现,如果该方法不存在添加该方法
 ...
 */
#pragma mark - 创建继承自NSObject类的People类
 ...
 */
#pragma mark - 将People类注册到runtime中
 ...
 */
#pragma mark - 获取方法的Type字符串(包含参数类型和返回值类型)
 ...
 *  \c imp_removeBlock.
 */
#pragma mark - 通过块创建函数指针,block的形式为^ReturnType(id self,参数,...)
...
#endif_START_COPYRIGHT__JIANSHU_BAIKAISHUILN__WechatPublic_Codeidea__END
